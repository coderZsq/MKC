...
| 覆盖用例 ID | 测试名称 | 说明 |
|---|---|---|
| MKC-TC-S2-4-001 | upload PDF creates a pdf_parse task | 直接调用 Gateway 上传 PDF，验证创建 `pdf_parse` 任务并返回 task_id |
...
注意：本 E2E 测试不启动 AI Service 与 Celery Worker，因此不验证 PyMuPDF 实际文本提取、目录解析、扫描件检测及进度上报等后端处理逻辑。
